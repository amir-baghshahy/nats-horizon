...
export default function ConnectionStatus({
  ...
  const serverCount = new Set(
    list.map((c) => c.server || c.server_id || "unknown"),
  ).size;
    ...
    <div className="card h-full flex flex-col min-h-0">
      ...
      <div className="flex items-center justify-between mb-3 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-display-sm font-semibold leading-tight">
      ...
      <div className="grid grid-cols-4 gap-1.5 mb-3 shrink-0">
        {[
          ...
          <div
            key={label}
            className="rounded-lg bg-surface-primary/50 px-2 py-2 text-center"
          >
            <p className="text-display-xs text-content-tertiary leading-tight">
              {label}
            </p>
            <p className="text-display-sm font-semibold tabular-nums mt-0.5">
              {value}
            </p>
          </div>
        ))}
      ...
      {list.length > 0 && (
        <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin flex flex-col justify-start pe-0.5">
          <div className="space-y-1">
            {visible.map((conn) => (
              <ConnectionRow
                key={conn.cid ?? conn.ip ?? conn.name ?? Math.random()}
                conn={conn}
                t={t}
              />
            ))}
                ...
                {expanded ? (
                  ...
                  <>
                    ...
                    {t("dashboard.showMore", {
                      count: list.length - INITIAL_VISIBLE,
                    })}
                  </>
                )}
              </button>
...
function ConnectionRow({ conn, t }: { conn: ConnectionInfo; t: TranslateFn }) {
  ...
  return (
    <div className="rounded-lg border border-border-default/40 bg-surface-primary/20 overflow-hidden">
      <button
        onClick={() => setOpen((p) => !p)}
        className="flex w-full items-center gap-2.5 px-3 py-2 text-left hover:bg-surface-primary/50 transition-colors"
      >
        <span className="h-1.5 w-1.5 rounded-full bg-green-400 animate-pulse shrink-0" />

        <Server className="h-3.5 w-3.5 text-content-tertiary shrink-0" />

        <div className="flex-1 min-w-0 flex items-center gap-2">
          <span className="text-display-xs font-medium truncate">{label}</span>
          {addr && (
            <span
              className="text-display-xs font-mono text-content-tertiary/70 truncate hidden sm:block"
              title={addr}
            >
              {addr}
            </span>
        ...
        <div className="hidden sm:flex items-center gap-1 shrink-0 text-display-xs text-content-tertiary">
          <span>
            {conn.subs_count || 0} {t("connections.subs")}
          </span>
        </div>

        <ChevronDown
          className={`h-3 w-3 text-content-tertiary shrink-0 transition-transform duration-150 ${open ? "rotate-180" : ""}`}
        />
      </button>
      ...
      {open && (
        <div className="border-t border-border-default/40 px-3 py-2 grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1.5 text-display-xs">
          ...
          <DetailRow
            label={t("dashboard.subscriptions")}
            value={conn.subs_count ?? 0}
          />
          <DetailRow
            label={t("dashboard.pending")}
            value={formatNumber(conn.pending_bytes)}
          />
          <DetailRow
          ...
          {conn.server && (
            <div className="col-span-full border-t border-border-default/30 pt-1.5 mt-0.5 flex items-center gap-2">
              <span className="text-content-tertiary shrink-0">
                {t("dashboard.natsServer")}
              </span>
              <span
                className="font-mono text-display-xs text-content-tertiary/70 truncate"
                title={conn.server}
              >
                {conn.server}
              </span>
