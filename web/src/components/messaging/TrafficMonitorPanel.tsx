...
export default function TrafficMonitorPanel({
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <Activity
                className={`h-5 w-5 text-primary-400 ${isMonitoring ? "animate-spin-slow" : ""}`}
              />
              <h2 className="text-display-xl font-bold">
                {t("messages.trafficMonitor")}
              </h2>
              {isMonitoring && (
      ...
      <div
        className={`grid gap-3 md:grid-cols-4 ${isMonitoring ? "animate-border-flow rounded-xl" : ""}`}
      >
        <div className="card p-3">
          ...
          <p
            className={`mt-1 text-display-lg font-bold ${isMonitoring ? "text-green-400 animate-live-glow" : ""}`}
          >
        ...
        <div className="card p-3">
          ...
          <p className="mt-1 text-display-lg font-bold">
            {subjectStats.length}
          </p>
        </div>
        <div className="card p-3">
          <p className="text-display-xs text-content-tertiary">
            {t("messages.messagesLabel")}
          </p>
          <p className="mt-1 text-display-lg font-bold">
      ...
      <div className="grid gap-4 md:grid-cols-2 flex-1 min-h-0">
        <div className="card overflow-hidden flex flex-col h-full">
          <div className="border-b border-border-default p-3">
            <h3 className="font-semibold text-display-sm">
              {t("messages.subjectTraffic")}
            </h3>
          </div>
                ...
                <div key={stat.subject} className="p-3">
                  <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0">
                      <p className="font-mono text-display-sm font-medium truncate">
                        {stat.subject}
                      </p>
                      <p className="mt-0.5 text-display-xs text-content-tertiary">
                        {t("messages.lastSeen")}{" "}
                        {formatTimestamp(stat.last_seen)}
                      </p>
                    </div>
