...
export default function KVStore() {
  ...
  const { data: buckets, refetch: refetchBuckets } = useQuery({
    queryKey: ['kvBuckets'],
    queryFn: () => axios.get('/api/kv/buckets').then(res => res.data),
    refetchInterval: 10000,
  })

  const { data: keys } = useQuery({
    queryKey: ['kvKeys', selectedBucket],
    queryFn: () => axios.get(`/api/kv/buckets/${selectedBucket}/keys`).then(res => res.data),
    enabled: !!selectedBucket,
  })

  const { data: history } = useQuery({
    queryKey: ['kvHistory', selectedBucket, selectedKey],
    queryFn: () => axios.get(`/api/kv/buckets/${selectedBucket}/history?key=${selectedKey}`).then(res => res.data),
    enabled: !!selectedBucket && !!selectedKey && showHistoryModal,
  })

  const createBucketMutation = useMutation({
    mutationFn: (data: any) => axios.post('/api/kv/buckets', data),
    onSuccess: () => {
  ...
  const putKeyMutation = useMutation({
    mutationFn: ({ key, value }: { key: string; value: string }) =>
      axios.put(`/api/kv/buckets/${selectedBucket}/key?key=${key}`, { key, value }),
    onSuccess: () => {
  ...
  const deleteKeyMutation = useMutation({
    mutationFn: (key: string) => axios.delete(`/api/kv/buckets/${selectedBucket}/key?key=${key}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['kvKeys', selectedBucket] })
    },
  })
  ...
  const filteredKeys = keys?.filter((k: KeyValue) =>
        ...
        <div className="lg:col-span-1">
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => refetchBuckets()} className="p-2 hover:bg-dark-bg rounded-lg">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-dark-muted">
                        {bucket.values?.toLocaleString() || 0} keys • {formatBytes(bucket.bytes || 0)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-dark-muted">{buckets?.find((b: Bucket) => b.name === selectedBucket)?.bucket_name}</p>
                </div>
              ...
              <div className="space-y-2 max-h-[600px] overflow-y-auto">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-xs text-dark-muted">v{kv.revision}</span>
                        </div>
                ...
                {filteredKeys.length === 0 && (
                  <div className="text-center py-8 text-dark-muted">
                    ...
                    <p>{searchQuery ? 'No keys match your search' : 'No keys in this bucket'}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card max-w-md w-full">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowCreateModal(false)} className="p-2 hover:bg-dark-bg rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={(e) => {
              ...
              <div>
                <label className="block text-sm font-medium mb-2">History (Revisions)</label>
                <input
                  type="number"
                  name="history"
                  defaultValue={1}
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Max Bytes (0 = unlimited)</label>
                <input
                  type="number"
                  name="max_bytes"
                  placeholder="1048576"
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card max-w-lg w-full">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowKeyModal(false)} className="p-2 hover:bg-dark-bg rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
          ...
          <div className="card max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-dark-muted font-mono">{selectedKey}</p>
              </div>
              <button onClick={() => setShowHistoryModal(false)} className="p-2 hover:bg-dark-bg rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto space-y-2">
              {history?.map((entry: KeyHistory) => (
                <div key={entry.revision} className="p-4 bg-dark-bg rounded-lg">
