...
import {
  Search, Send, Trash2, Eye, RefreshCw, Filter, Download,
  ChevronDown, ChevronRight, Maximize2, Code, FileText, Clock,
...
export default function Messages() {
  ...
  const publishMutation = useMutation({
    mutationFn: (data: { stream: string; subject: string; data: string }) =>
      axios.post(`/api/streams/${encodeURIComponent(data.stream)}/messages/publish`, {
  ...
  const defaultStream = selectedStream || streamList[0]?.config?.name || streamList[0]?.name || ''
  ...
  const { data: messages, refetch: refetchMessages } = useQuery({
    ...
    queryFn: () => axios.get(`/api/messages?stream=${encodeURIComponent(selectedStream)}`).then(res => res.data),
    ...
    <div className="p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-dark-muted mt-1">View, publish, and manage stream messages</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowPublishModal(true)} className="btn-primary flex items-center gap-2">
            ...
            >
              {streams?.map((stream: any) => (
                <option key={stream.config?.name || stream.name} value={stream.config?.name || stream.name}>
                  {stream.config?.name || stream.name} ({stream.state?.messages?.toLocaleString()} messages)
                </option>
              ))}
            </select>
          </div>
      ...
      <div className="card mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <span className="text-sm text-dark-muted">Showing {displayMessages.length} messages</span>
            <select
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => refetchMessages()} className="btn-secondary text-sm py-1 px-3 flex items-center gap-2">
              <RefreshCw className="w-4 h-4" />
              Refresh
            </button>
          </div>
      ...
      <div className="card overflow-hidden p-0">
        ...
        <div className="bg-dark-bg border-b border-dark-border p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <input
                type="checkbox"
                checked={selectedMessages.size === displayMessages.length && displayMessages.length > 0}
                onChange={toggleAll}
        ...
        <div className="divide-y divide-dark-border">
          {displayMessages.map((message: any) => {
            ...
            return (
              <div key={sequence} className="border-l-2 border-l-transparent hover:border-l-primary-500 transition-colors">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="font-mono text-sm text-primary-400">#{sequence}</span>
                        <span className="text-sm font-medium">{message.subject}</span>
                        <span className="text-xs text-dark-muted">{formatBytes(message.size || 0)}</span>
                    ...
                    <div className="mt-4 pl-8 space-y-4">
                      ...
                      <div className="bg-dark-bg/50 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          >
                            ...
                            {expandedMessages.has(sequence) ? 'Collapse' : 'Expand'}
                          </button>
                        </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="bg-dark-bg/50 rounded-lg p-3">
                          ...
                          <p className="text-sm">{new Date(message.timestamp || Date.now()).toLocaleString()}</p>
                        </div>
                        <div className="bg-dark-bg/50 rounded-lg p-3">
                      ...
                      <div className="flex items-center gap-3 pt-2">
                        <button
                          onClick={() => {
                            ...
                            toast('info', 'Set the subject and payload to reply')
                          }}
          ...
          <div className="card max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            >
              <div>
                ...
                <select name="stream" className="input w-full" defaultValue={selectedStream}>
                  {streams?.map((stream: any) => (
                    <option key={stream.config?.name || stream.name} value={stream.config?.name || stream.name}>
              ...
              <div className="flex items-center gap-3 pt-4">
                ...
                <button type="submit" disabled={publishMutation.isPending} className="btn-primary flex items-center gap-2">
                  <Send className="w-4 h-4" />
