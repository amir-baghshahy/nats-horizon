...
export default function ClusterPage({
  ...
  if (
    ...
    healthLoading ||
    streamsLoading ||
    (selectedStream && replicasLoading)
  ) {
    ...
    return (
      <PageError
        message={getErrorMessage(
          infoError || nodesError || healthError || streamsError,
        )}
        onRetry={refreshAll}
      />
    );
    ...
    <div className="flex flex-col gap-4 p-4 md:p-6 md:h-full md:overflow-hidden">
        ...
        <PageHeader
          ...
          subtitle={
            clusterInfo?.is_clustered
              ? t("cluster.cluster", {
                  name: clusterInfo?.cluster_name || t("cluster.notClustered"),
                })
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">
                {clusterHealth?.connected
                  ? t("cluster.connected")
                  : t("cluster.disconnected")}
              </p>
              <p className="text-display-sm text-content-tertiary">
                {clusterHealth?.connected
                  ? t("cluster.server", {
                      name:
                        clusterHealth?.connected_server?.id || "Current server",
                    })
          ...
          >
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center p-3 bg-surface-primary/50 rounded-lg">
                ...
                <span
                  className={`px-2 py-1 rounded text-display-xs ${
                    clusterInfo?.is_clustered
          ...
          >
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center p-3 bg-surface-primary/50 rounded-lg">
                ...
                >
                  {clusterHealth?.jetstream?.status ||
        ...
        {clusterNodes?.clustered && (clusterNodes.nodes ?? []).length > 0 && (
          <PanelCard
            title={t("cluster.clusterNodes", {
              count: (clusterNodes.nodes ?? []).length,
            })}
                ...
                >
                  ...
                  <div className="space-y-2 text-display-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-display-xs ${
                          node.healthy
                            ? "bg-status-success/20 text-status-success"
                            : "bg-status-error/20 text-status-error"
                        }`}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-display-xs text-content-tertiary">
                          {stream.config?.replicas}x{" "}
                          {t("cluster.replicas", {
                            count: stream.config?.replicas,
                          })}
                        </span>
                      </div>
                    </div>
              ...
              {selectedStream && streamReplicas && (
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    {streamReplicas.mirror && (
                      <div className="p-4 bg-purple-500/10 rounded-lg border border-purple-500/30">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-medium">
                            {t("cluster.mirrorSource")}
                          </span>
                        </div>
                        <p className="font-mono text-display-sm">
                          {streamReplicas.mirror.name}
                        </p>
                    ...
                    {streamReplicas.sources &&
                      streamReplicas.sources.length > 0 && (
                        <div className="p-4 bg-blue-500/10 rounded-lg border border-blue-500/30">
                          <div className="flex items-center gap-2 mb-2">
                            <Globe className="icon-base text-blue-400" />
                            <span className="font-medium">
                              {t("cluster.aggregateSources")}
                            </span>
                          </div>
                          <div className="space-y-1">
                            {streamReplicas.sources.map(
                              (source: any, idx: any) => (
                                <p
                                  key={idx}
                                  className="font-mono text-display-sm"
                                >
                                  {source.name}
                                  {source.domain && (
                    ...
                    {streamReplicas.placement && (
                      <div className="p-4 bg-surface-primary/50 rounded-lg">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-medium">
                            {t("cluster.placementRules")}
                          </span>
                        </div>
                        <p className="text-display-sm text-content-tertiary">
                          {t("cluster.title")}:{" "}
                          <span className="font-mono">
                            {streamReplicas.placement?.cluster}
                          </span>
                        </p>
                        {(streamReplicas.placement?.tags ?? []).length > 0 && (
                          <div className="mt-2">
                            ...
                            {(streamReplicas.placement?.tags ?? []).map(
                              (tag: any, idx: any) => (
                                <span
                                  key={idx}
                                  className="inline-block px-2 py-0.5 bg-surface-primary rounded text-display-xs font-mono ml-1"
                                >
                                  {tag}
                                </span>
                              ),
