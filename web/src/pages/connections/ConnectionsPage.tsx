    ...
    <div className="flex flex-col gap-4 p-4 md:p-6 md:h-full md:overflow-hidden">
      ...
      <div className="shrink-0 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <StatCard
          ...
          sub={t("connections.msgsCount", {
            count: formatNumber(stats.totalMsgsIn),
          })}
        />
        <StatCard
          icon={Upload}
          ...
          sub={t("connections.msgsCount", {
            count: formatNumber(stats.totalMsgsOut),
          })}
        />
      </div>
              ...
              {serverData.map((server: any) => {
                const percentage =
                  filteredConnections.length > 0
                    ? Math.round(
                        (server.connections / filteredConnections.length) * 100,
                      )
                    : 0;
      ...
      <DataList
        ...
        footer={
          <span>
            {t("connections.showingConnections", {
                  ...
                  <div className="hidden lg:flex items-center gap-4 text-display-sm">
                    <div className="text-center min-w-[52px]">
                      <p className="font-medium tabular-nums">
                        {formatNumber(conn.subs_count || 0)}
                      </p>
                    ...
                    <div className="text-center min-w-[64px]">
                      <p className="font-medium tabular-nums flex items-center justify-center gap-1">
                        ...
                        {formatNumber(conn.in_msgs || 0)}
                      </p>
                      <p className="text-display-xs text-content-tertiary">
                        {t("connections.inMsgs")}
                      </p>
                    </div>
                    <div className="text-center min-w-[64px]">
                      <p className="font-medium tabular-nums flex items-center justify-center gap-1">
                        <Upload className="h-3 w-3 text-violet-400" />
                        {formatNumber(conn.out_msgs || 0)}
                      </p>
                      <p className="text-display-xs text-content-tertiary">
                        {t("connections.outMsgs")}
                      </p>
                    </div>
                    ...
                    <button
                      onClick={async () => {
                        const ok = await confirm({
                          ...
                          message: t("connections.terminateConfirm", {
                            cid: String(conn.cid),
                          }),
                        ...
                        if (ok) {
                          HealthService.deleteConnections(
                            ...
                            .then(() => refetch())
                            .catch(() =>
                              toast("error", t("connections.terminateFailed")),
                            );
                        }
                      }}
                ...
                {isExpanded && (
                  <div className="mt-4 md:pl-8">
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                      <DetailItem
                        icon={Download}
                        label={t("connections.inMsgs")}
                        value={formatNumber(conn.in_msgs || 0)}
                      />
                      <DetailItem
                        icon={Upload}
                        label={t("connections.outMsgs")}
                        value={formatNumber(conn.out_msgs || 0)}
                      />
                      <DetailItem
                        icon={Download}
                        label={t("connections.inBytes")}
                        value={formatBytes(conn.in_bytes || 0)}
                      />
                      <DetailItem
                        icon={Upload}
                        label={t("connections.outBytes")}
                        value={formatBytes(conn.out_bytes || 0)}
                      />
                      <DetailItem
                        icon={Activity}
                        label={t("connections.subs")}
                        value={formatNumber(conn.subs_count || 0)}
                      />
                      <DetailItem
                        icon={Gauge}
                        label={t("connections.pending")}
                        value={formatBytes(conn.pending_bytes || 0)}
                      />
                      <DetailItem
                        icon={Gauge}
                        label={t("connections.rtt")}
                        value={conn.rtt || t("dashboard.na")}
                      />
                      <DetailItem
                        icon={Hash}
                        label={t("connections.cid")}
                        value={
                          conn.cid != null ? `#${conn.cid}` : t("dashboard.na")
                        }
                      />
                      <DetailItem
                        icon={Clock}
                        label={t("connections.connectedSince")}
                        value={
                          conn.connected_at
                      ...
                      <DetailItem
                        icon={Clock}
                        label={t("connections.lastActivity")}
                        value={
                          conn.last_activity
                            ? new Date(conn.last_activity).toLocaleString()
                            : t("dashboard.na")
                        }
                      />
                      <DetailItem
                        icon={Server}
                        label={t("connections.serverId")}
                        value={conn.server_id || t("dashboard.na")}
                        mono
                      />
                      {conn.name && (
                        <DetailItem
                          icon={Cable}
                          label={t("connections.clientName")}
                          value={conn.name}
                          mono
                        />
                      )}
...
function DetailItem({
  ...
  return (
    <div className="bg-surface-primary/50 rounded-lg p-3 min-w-0">
      ...
      <p
        className={`font-medium text-display-sm mt-1 truncate tabular-nums ${mono ? "font-mono" : ""}`}
