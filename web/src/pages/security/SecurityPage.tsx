...
export default function SecurityPage({
  ...
  if (activeError) {
    return (
      <PageError message={getErrorMessage(activeError)} onRetry={refetchInfo} />
    );
  }
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-surface-primary/50 rounded-lg p-4">
                  <p className="text-display-xs text-content-tertiary">
                    {t("security.accountName")}
                  </p>
                  <p className="font-medium">
                    {securityInfo?.account?.name || "N/A"}
                  </p>
                </div>
                <div className="bg-surface-primary/50 rounded-lg p-4">
                  <p className="text-display-xs text-content-tertiary">
                  ...
                  <p className="font-medium">
                    {securityInfo?.account?.imports || 0}
                  </p>
                </div>
                <div className="bg-surface-primary/50 rounded-lg p-4">
                  <p className="text-display-xs text-content-tertiary">
                  ...
                  <p className="font-medium">
                    {securityInfo?.account?.exports || 0}
                  </p>
                </div>
                <div className="bg-surface-primary/50 rounded-lg p-4">
                  <p className="text-display-xs text-content-tertiary">
                  ...
                  <p className="font-medium text-display-sm">
                    {securityInfo?.timestamp
                      ? formatDateTime(securityInfo.timestamp)
                      : "N/A"}
                  </p>
                </div>
              </div>
            ...
            <PanelCard title={t("security.resourceLimits")}>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-surface-primary/50 rounded-lg p-4">
                  <p className="text-display-xs text-content-tertiary">
                    {t("security.maxConnections")}
                  </p>
                  <p className="font-medium">
                    {securityInfo?.limits?.connections || "Unlimited"}
                  </p>
                </div>
                <div className="bg-surface-primary/50 rounded-lg p-4">
                  <p className="text-display-xs text-content-tertiary">
                    {t("security.maxSubscriptions")}
                  </p>
                  <p className="font-medium">
                    {securityInfo?.limits?.subscriptions || "Unlimited"}
                  </p>
                </div>
                <div className="bg-surface-primary/50 rounded-lg p-4">
                  <p className="text-display-xs text-content-tertiary">
                  ...
                  <p className="font-medium">
                    {securityInfo?.limits?.data
                      ? formatBytes(securityInfo.limits.data)
                      : "Unlimited"}
                  </p>
                </div>
                <div className="bg-surface-primary/50 rounded-lg p-4">
                  <p className="text-display-xs text-content-tertiary">
                    {t("security.maxPayload")}
                  </p>
                  <p className="font-medium">
                    {securityInfo?.limits?.payload
                      ? formatBytes(securityInfo.limits.payload)
                      : "Unlimited"}
                  </p>
                </div>
              </div>
            ...
            <PanelCard title={t("security.serverSecuritySettings")}>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-surface-primary/50 rounded-lg">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-display-sm ${
          ...
          >
            <div className="space-y-4">
              {users?.map((user) => (
                <div
                  key={user.name}
                  className="p-4 bg-surface-primary/50 rounded-lg"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span
                          className={`text-display-xs px-2 py-1 rounded ${
                            user.enabled
                              ? "bg-green-500/20 text-green-400"
                      ...
                      <p className="text-display-sm text-content-tertiary mb-2">
                        {t("security.account", { name: user.account })}
                      </p>
                      <div className="grid grid-cols-2 gap-4 text-display-xs">
                        <div>
                          <p className="text-content-tertiary">
                            {t("security.publishPermissions")}
                          </p>
                          <div className="mt-1 space-y-1">
                            {Object.entries(
                              user.permissions?.publish ??
                                ({} as Record<string, string>),
                            ).map(([subject, perm]: [string, string]) => (
                              <div
                                key={subject}
                                className="font-mono bg-surface-primary px-2 py-1 rounded"
                              >
                                {subject}: {perm}
                              </div>
                            ))}
                        ...
                        <div>
                          <p className="text-content-tertiary">
                            {t("security.subscribePermissions")}
                          </p>
                          <div className="mt-1 space-y-1">
                            {Object.entries(
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        onClick={async () => {
                          const ok = await confirm({
                            title: user.enabled
                              ? t("security.disableUser")
                              : t("security.enableUser"),
                            message: t("security.toggleUserConfirm", {
                              name: user.name,
                            }),
                            confirmLabel: user.enabled
                              ? t("security.disable")
                              : t("security.enable"),
                            variant: "warning",
                          });
                      ...
                      >
                        {user.enabled ? (
                          <ToggleRight className="icon-base text-green-400" />
                        ) : (
                          <ToggleLeft className="icon-base text-content-tertiary" />
                        )}
                      </button>
                      <button
                        type="button"
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const ok = await confirm({
                            ...
                            message: t("security.deleteUserConfirm", {
                              name: user.name,
                            }),
                            confirmLabel: t("security.delete"),
                            variant: "danger",
                          });
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <span className="text-display-sm text-content-tertiary">
                          {t("security.byUser", { name: log.user })}
                        </span>
                      </div>
                      <p className="text-display-sm text-content-tertiary">
                        {t("security.resource", { name: log.resource })}
                      </p>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h2 id="user-modal-title" className="text-display-sm font-bold">
                  {selectedUser
                    ? t("security.editUser")
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    defaultValue={
                      selectedUser
                        ? Object.keys(
                            selectedUser.permissions?.publish ?? {},
                          ).join(", ")
                        : ">"
                    }
                  />
                  <p className="text-display-xs text-content-tertiary mt-1">
                    {t("security.permissionsHelp")}
                  </p>
                </div>
                <div>
                  <label className="block text-display-sm font-medium mb-2">
                  ...
                  <input
                    ...
                    defaultValue={
                      selectedUser
                        ? Object.keys(
                            selectedUser.permissions?.subscribe ?? {},
                          ).join(", ")
                        : ">"
                    }
                  />
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    id="user-enabled"
                    name="enabled"
                    defaultChecked={selectedUser?.enabled ?? true}
                  />
                  <label htmlFor="user-enabled" className="text-display-sm">
                    {t("security.enableUserCheckbox")}
                  </label>
                </div>
