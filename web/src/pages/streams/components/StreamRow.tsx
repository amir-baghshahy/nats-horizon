        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-display-xs text-content-tertiary">
                {stream.config?.storage === "file" ? t("streams.file") : t("streams.memory")}
              </span>
            </div>

            <div className="flex items-center gap-4 text-display-sm text-content-tertiary">
              <span>{t("streams.messagesCount", { count: stream.state?.messages || 0, defaultValue: `${(stream.state?.messages || 0).toLocaleString()} messages` })}</span>
              <span>{formatBytes(stream.state?.bytes || 0)}</span>
              <span>{t("streams.consumersCount", { count: stream.state?.consumers || 0, defaultValue: `${stream.state?.consumers || 0} consumers` })}</span>

              {(stream.state?.num_pending || 0) > 0 && (
                <span className="text-yellow-400">
                  {t("streams.pendingCount", { count: stream.state?.num_pending || 0, defaultValue: `${stream.state?.num_pending?.toLocaleString()} pending` })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 hover:bg-surface-primary rounded-lg hover-lift active-scale transition-colors"
              title={t("streams.exportStream", { defaultValue: "Export stream" })}
            >
            ...
            <button
              ...
              className="p-2 hover:bg-surface-primary rounded-lg hover-lift active-scale transition-colors"
              title={t("streams.exportMessages", { defaultValue: "Export messages" })}
            >
            ...
            <button
              ...
              className="p-2 hover:bg-surface-primary rounded-lg hover-lift active-scale transition-colors"
              title={t("streams.purgeMessages", { defaultValue: "Purge messages" })}
            >
            ...
            <button
              ...
              title={t("streams.deleteStream", { defaultValue: "Delete stream" })}
            >
        ...
        {isItemExpanded && (
          <div className="mt-4 pl-10 grid grid-cols-1 md:grid-cols-3 gap-4 text-display-sm animate-fade-in-down">
            <div className="bg-surface-primary/50 rounded-lg p-3 hover-scale">
              <p className="text-display-xs text-content-tertiary mb-1">{t("streams.subjects")}</p>
            ...
            <div className="bg-surface-primary/50 rounded-lg p-3 hover-scale">
              <p className="text-display-xs text-content-tertiary mb-1">{t("streams.configuration", { defaultValue: "Configuration" })}</p>
              <div className="space-y-1">
                <p>{t("streams.replicas")}: {stream.config?.replicas}</p>
                <p>{t("streams.retention")}: {stream.config?.retention}</p>
                <p>{t("streams.maxAge")}: {stream.config?.max_age}</p>
              </div>
            </div>

            <div className="bg-surface-primary/50 rounded-lg p-3 hover-scale">
              <p className="text-display-xs text-content-tertiary mb-1">{t("streams.state", { defaultValue: "State" })}</p>
              <div className="space-y-1">
                <p>{t("streams.firstSeq", { defaultValue: "First Seq" })}: {stream.state?.first_seq?.toLocaleString() || 0}</p>
                <p>{t("streams.lastSeq", { defaultValue: "Last Seq" })}: {stream.state?.last_seq?.toLocaleString() || 0}</p>
              </div>
            </div>
          </div>
        )}
