...
export interface UseTenancyReturn {
  ...
  setEditingConnection: React.Dispatch<React.SetStateAction<ConnectionConfig | null>>
  testingUrl: string
  testResult: ConnectionTestResult | null
  setTestResult: React.Dispatch<React.SetStateAction<ConnectionTestResult | null>>
...
export function useTenancy(): UseTenancyReturn {
  ...
  const { data: statuses, isLoading: statusesLoading, error: statusesError } = useQuery({
    ...
    queryFn: () => TenancyService.getTenancyStatus() as unknown as Promise<{ statuses: ConnectionStatus[] }>,
  ...
  const setDefaultMutation = useMutation({
    mutationFn: (id: string) => TenancyService.getTenancyConnectionsDefault(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tenancyConnections'] })
    },
  })
  ...
  const testConnectionMutation = useMutation({
    mutationFn: (url: string) => TenancyService.postTenancyConnectionsTest({ url }),
    onSuccess: (data) => {
      setTestResult(data as ConnectionTestResult)
    },
  })

  const getStatusForConnection = useCallback((id: string) => {
